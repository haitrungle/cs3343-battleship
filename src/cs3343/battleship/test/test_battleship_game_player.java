...
public class test_battleship_game_player {
    ...
    public void test_battleship_game_player_2() throws Exception {
        ...
        for(int i = 0; i < positions.size();i++){
            assertTrue(positions.get(i).equals(d==Direction.DOWN? new Position(row+i,col) : new Position(row,col+i)));
        }
    }
    ...
    public void test_battleship_game_player_6() throws Exception {
        ...
        assertEquals(Console.textColor("Invalid input: ", Console.RED) +"You have already shot at " + p + ". Please select another position.", e.getMessage());
    }
    ...
    public void test_battleship_game_player_8() throws Exception {
        ...
        assertEquals(Console.textColor("Invalid input: ", Console.RED) +"Position (" + "-1" + "," + 0 + ") is out of bounds. Row and column must be between 0 and " + (Config.BOARD_SIZE - 1) + ".", e.getMessage());
    }
